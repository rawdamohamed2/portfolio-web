...
export default function Education() {
    let [experience,setExperiences]=useState([]);
    async function getExperience(){
        try {
            const experience = await axios.get('https://raw.githubusercontent.com/rawdamohamed2/my-Portfolio-data/main/education.json');
    ...
    if(experience.length == 0 || !experience){
        return <Loader/>
    }
    console.log(experience);
  return (
      <Element name="education"  className=" bg-MColor py-12 px-6 overflow-hidden">
              ...
              <div className="relative max-w-5xl mx-auto py-5">
                  ...
                  <div className="space-y-12">
                      {experience.map((item, index) => (
                          ...
                          <div key={index} className={`relative  flex items-center justify-between w-full mb-8  ${index % 2 === 0 ? 'md:flex-row-reverse' : 'flex-row'}`} data-aos="fade-up" data-aos-delay="500" >
                              ...
                              <div className=' p-4 w-full md:w-[45%] bg-cricleColor hover:bg-transparent hover:shadow-[0px_0px_15px_#00FFF7] transform hover:scale-[1.06] rounded-xl transition-all duration-500'>
                                  ...
                                  <p className="font-light md:mt-4 mt-1 md:mb-1 mb-0 text-[1.03rem]">{item.description}</p>
                                  <p className="font-light mb:md-1 mb-0 text-[1.03rem]">{item.company} <span className="text-effectColor">|</span> [ {item.startDate} - {item.endDate} ]</p>
                                  {item.gpa?<p className="font-light mb:md-1 mb-0 text-[1.03rem]">GPA: <span
                                      className=""> 3.73 </span></p>:''}
                              </div>
                          </div>
                      ))}
